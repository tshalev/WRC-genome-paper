...
all_snps_scaffolds_clusters_pi <- read.table("all_snps_scaffold_clusters/all_snps_scaffold_clusters_pi.txt", header = T)

# Pi SCO genes for whole pop, clusters
all_snps_sco_genes_one_pop_pi <- read.table("all_snps_primary_sco_gene_one_pop/all_snps_primary_sco_gene_one_pop_pi.txt", header = T)
all_snps_sco_genes_clusters_pi <- read.table("all_snps_sco_genes_clusters/all_snps_sco_genes_clusters_pi.txt", header = T)
...
all_snps_sco_genes_clusters_dxy <- read.table("all_snps_sco_genes_clusters/all_snps_sco_genes_clusters_dxy.txt",header = T)
...
all_snps_sco_genes_one_pop_pi %>% 
  filter(no_sites >= 10) %>% 
  group_by(pop) %>%
  summarise(across(avg_pi, list(mean = mean, median = median, sd = sd)))

all_snps_sco_genes_clusters_pi %>% 
  filter(no_sites >= 10) %>% 
  group_by(pop) %>%
  summarise(across(avg_pi, list(mean = mean, median = median, sd = sd)))

all_snps_sco_genes_clusters_dxy <- all_snps_sco_genes_clusters_dxy %>% 
  filter(no_sites >= 10) %>% 
  unite(pop_comparison, c("pop1", "pop2"))

all_snps_sco_genes_clusters_dxy %>% 
...
all_snps_sco_genes_one_pop_pi$chromosome <- factor(all_snps_sco_genes_one_pop_pi$chromosome)

# Top 1% of pi estimates
top_1 <- all_snps_sco_genes_one_pop_pi[all_snps_sco_genes_one_pop_pi$avg_pi > quantile(all_snps_sco_genes_one_pop_pi$avg_pi,prob=1-1/100),]

# Figure 3A
ggplot(all_snps_sco_genes_one_pop_pi, aes(x = chromosome, y = avg_pi, color = chromosome)) +
...
# Figure 3B
ggplot(all_snps_sco_genes_one_pop_pi, aes(x = pop, y = avg_pi)) +
  geom_violin() +
...
# Figure 3C
ggplot(all_snps_sco_genes_clusters, aes(x = pop_comparison, y = avg_dxy)) +
  geom_violin() +
  geom_boxplot(outlier.alpha = 0.3, width = 0.15) +
  theme_Publication() +
...
all_snps_0_4_gff <- rbind(data.frame(pi = all_snps_0_fold_gff_one_pop_pi$avg_pi, degen = rep("zero", 2836)), data.frame(pi = all_snps_4_fold_gff_one_pop_pi$avg_pi, degen = rep("four", 2235)))
...
# Load annotations
gene_annotations <- read.table("../../../../../all_GAPP_data/new_data/Tplicata.v3.1.prelim.annot/Tplicata.v3.1.annot/gene.functions_dashes_removed.txt", skip = 1, header = F,
                               sep = "\t")
...
top_1_annotations <- merge(top_1_gff, gene_annotations, by = "Gene", all = T) 
...
# Pi
NC_cluster_pi <- all_snps_sco_genes_clusters_pi %>% 
  filter(pop == "Northern_Coastal")

C_cluster_pi <- all_snps_sco_genes_clusters_pi %>% 
  filter(pop == "Central")

SI_cluster_pi <- all_snps_sco_genes_clusters_pi %>% 
...
# dxy
NC_C_cluster_dxy <- all_snps_sco_genes_clusters %>% 
  filter(pop_comparison == "Northern_Coastal_Central")

C_SI_cluster_dxy <- all_snps_sco_genes_clusters %>% 
  filter(pop_comparison == "Central_Southern_Interior")

NC_SI_cluster_dxy <- all_snps_sco_genes_clusters %>% 
