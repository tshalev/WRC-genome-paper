...
write.table(melted_expectations_halving, "melted_expectations_halving.txt", quote = F, row.names = F)
write.table(melted_generation_FS_S5_hobs, "melted_generation_FS_S5_hobs.txt", quote = F, row.names = F)

# Read in simplified hets with no expected het for FS
...
FS_S5_hets %>% 
  filter(Heterozygosity == "Expected") %>% 
  group_by(V2) %>% 
...
S1_test <- S1_het %>% 
  sign_test(V3 ~ 1, mu = 0.154, alternative = "less") %>% 
  add_significance()
S1_test
...
diverse_pop_vcf <- read.vcfR("diverse_pop.final.g95minQ30minmeanDP15maxmeanDP70AB28.HWE_het.samples_sorted.recode.vcf")
...
write.table(S4_fate_table_props_for_chisq, "S4_fate_table_chisq_analysis.txt", row.names = F, col.names = T, quote = F)
