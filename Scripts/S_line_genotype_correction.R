...
rare_alleles <- allele_freqs_F %>% 
  filter(frq < 0.03 & frq > 0)

F_lines %>% rownames_to_column %>%  filter(rowname %in% rare_alleles$SNP)
### S1 gen ##############################################################################################
...
rare_alleles_S1 <- allele_freqs_S1 %>% 
  rownames_to_column() %>% 
...
S2_27_6_with_snp_names <- Line_27_6_corrected %>% 
  rownames_to_column() %>% 
  select(1, 4)
...
S2_lines <- merge(S2_1_1_with_snp_names, S2_1_4_with_snp_names, by = "rowname", all = T)
S2_lines <- merge(S2_lines, S2_6_1_with_snp_names, by = "rowname", all = T)
...
S2_lines <- merge(S2_lines, S2_27_6_with_snp_names, by = "rowname", all = T)
...
S3_27_6_with_snp_names <- Line_27_6_corrected %>% 
  rownames_to_column() %>% 
  select(1, 5)
...
S3_lines <- merge(S3_1_1_with_snp_names, S3_1_4_with_snp_names, by = "rowname", all = T)
S3_lines <- merge(S3_lines, S3_6_1_with_snp_names, by = "rowname", all = T)
...
S3_lines <- merge(S3_lines, S3_27_6_with_snp_names, by = "rowname", all = T)
...
S4_27_6_with_snp_names <- Line_27_6_corrected %>% 
  rownames_to_column() %>% 
  select(1, 6)
...
S4_lines <- merge(S4_1_1_with_snp_names, S4_1_4_with_snp_names, by = "rowname", all = T)
S4_lines <- merge(S4_lines, S4_6_1_with_snp_names, by = "rowname", all = T)
...
S4_lines <- merge(S4_lines, S4_27_6_with_snp_names, by = "rowname", all = T)
...
S5_lines <- merge(S5_1_1_with_snp_names, S5_1_4_with_snp_names, by = "rowname", all = T)
S5_lines <- merge(S5_lines, S5_6_4_with_snp_names, by = "rowname", all = T)
...
n0 <- apply(S5_lines==0,1,sum,na.rm=T)
n1 <- apply(S5_lines==1,1,sum,na.rm=T)
n2 <- apply(S5_lines==2,1,sum,na.rm=T)
...
#allele_freqs_S5 <- Propfunc(S5_lines)
allele_freqs_S5 <- data.frame(SNP = rownames(S5_lines), frq = maf)


### All gens ########################################################################################### 
all_gen_allele_freqs <- data.frame(SNP = allele_freqs_F$SNP, 
                                   F_frq = allele_freqs_F$frq,
                                   S1_frq = allele_freqs_S1$frq,
                                   S2_frq = allele_freqs_S2$frq,
                                   S3_frq = allele_freqs_S3$frq,
                                   S4_frq = allele_freqs_S4$frq,
                                   S5_frq = allele_freqs_S5$frq)

all_gen_allele_freqs_melted <- melt(all_gen_allele_freqs)
